...
export function Signup(props: ISignupProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const response = await fetch('https://scissors-v0r0.onrender.com/api/v1/users/register', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className='flex flex-col w-full  items-center justify-center  my-20'>
        ...
        <form onSubmit={handleSubmit} className='flex flex-col' >
          ...
          <input value={username} onChange={(e) => setUsername(e.target.value)} className='flex border-2 border-primary px-5 py-3 rounded-md  w-[450px] ' type='text'  placeholder='username'  />
          ...
          <input value={retypePassword}  onChange={(e)=> setRetypePassword(e.target.value)} className='flex border-2 border-primary px-5 py-3 rounded-md  w-[450px] ' type='password'  placeholder='Retype Password' />
